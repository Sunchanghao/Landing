...
PX4Tracking::PX4Tracking(const ros::NodeHandle& nh, const ros::NodeHandle& nh_private):
  ...
  //订阅gazebo提供的模型位置的真值
    gazebo_sub = nh_private_.subscribe("/gazebo/model_states", 10, &PX4Tracking::gazebo_cb,this,ros::TransportHints().tcpNoDelay());
  //飞控得到的飞机的位置
...
* @param[out] 
**/
Eigen::Vector4d PX4Tracking::TrackingPidProcess2(Eigen::Vector3d &relative_pos,Eigen::Vector3d &expectPos,Eigen::Quaterniond &q_gazebo)
{
 // Eigen::Vector3d s_PidOut;
  Eigen::Vector4d s_PidOut;
  cout<<"目标位置："<<pos_drone_gazebo[0]<<" "<<pos_drone_gazebo[1]<<" "<<pos_drone_gazebo[2]<<endl;
  cout<<"相对位置差："<<relative_pos[0]<<" "<< relative_pos[1]<<" "<<relative_pos[2]<<endl;
	/*前后方向的pid控制，输出机体坐标系下y方向的速度控制*/
	
	s_PidItemY.difference = relative_pos[0]-expectPos[0];  //difference为比例控制项
	if(s_PidItemY.difference>5)
	   s_PidItemY.difference=5;
	...
	s_PidOut[3] = 1*(s_PidY.p*s_PidItemY.difference + s_PidY.d*s_PidItemY.differential + s_PidY.i*s_PidItemY.intergral);
    if(s_PidOut[3]>1.5)
	   s_PidOut[3]=1;
	if(s_PidOut[3]<-1.5)
	   s_PidOut[3]=1;
    ...
    //左右方向的pid控制，计算速度时暂时与s_PidY或s_PidZ系数相同
		s_PidItemRoll.difference =-expectPos[1]+relative_pos[1];
		if(s_PidItemRoll.difference>3)
	  s_PidItemRoll.difference=3;
  ...
  /*输出s_PidOut[3]为飞机坐标系下Y轴也就是飞机左右方向的速度,此处为roll*/
	s_PidOut[0] = s_PidY.p*s_PidItemRoll.difference + s_PidY.d*s_PidItemRoll.differential + s_PidY.i*s_PidItemRoll.intergral;
	    if(s_PidOut[0]>1.5)
	   s_PidOut[0]=1;
	if(s_PidOut[0]<-1.5)
	   s_PidOut[0]=-1;
	...
	s_PidItemZ.difference = relative_pos[2]+expectPos[2] ;
  ...
  /*s_PidOut[2]为飞机坐标系下Z轴也就是飞机上下方向的速度*/
	s_PidOut[2] = 1*(s_PidZ.p*s_PidItemZ.difference + s_PidZ.d*s_PidItemZ.differential + s_PidZ.i*s_PidItemZ.intergral);
    ...
    if(abs(s_PidItemZ.difference)<0.01)
	s_PidOut[2]=0;
}
/**
		...
		case TRACKING:
		    /*search检测到目标，跳到TRACKING*/
			{
				/*如果检测到目标，进入pid追踪函数TrackingPidProcess（）*/
					...
					desire_vel_ = TrackingPidProcess2(relative_pos, desire_pose_, q_gazebo); 
                    
				// if(ar_pose_[2] <= 0.3)
				// {
				// 	TrackingState = TRACKOVER;
				// 	cout << "TRACKOVER" <<endl;
				// }  //如果飞机和二维码的距离小于0.3,代表追踪结束
			    /*下面这条if语句的意思是实时保持为板外模式*/
...
void PX4Tracking::gazebo_cb(const gazebo_msgs::ModelStates::ConstPtr &msg)
{
       model_states = *msg;
        pos_drone_gazebo = Eigen::Vector3d(model_states.pose[2].position.x, model_states.pose[2].position.y, model_states.pose[2].position.z);//获取抓取模型所在位置
        q_gazebo = Eigen::Quaterniond(model_states.pose[2].orientation.w, model_states.pose[2].orientation.x, model_states.pose[2].orientation.y, model_states.pose[2].orientation.z);
        relative_pos = pos_drone_gazebo -px4_pose_;  //获取位置差
        relative_q = q_gazebo.z() - px4_q_z;
}
/*接收来自飞控的当前飞机位置*/                  
void PX4Tracking::Px4PosCallback(const geometry_msgs::PoseStamped::ConstPtr &msg)
{
    ...
    px4_q_z = msg->pose.orientation.z;
